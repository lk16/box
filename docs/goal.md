...
box runs Claude Code inside a disposable Docker sandbox (`sbx`) so an agent can work on a
...
5. Run the agent with the base prompt appended to its system prompt, followed by the project's own
...
effect, `box gen` writes a starter `.box/` directory, `box mount-prompt` prints the prompt that has
an agent on this host fill in the mounts `.box/mounts.json` still leaves as placeholders, and
`box self-update` installs the published release over the running one.

## Constraints

[README.md](../README.md) and the pages it links to are where what box does is written down, for
the people who use it. This file says why, and is the authority when the two disagree. So a rule
the README spells out — the settings table, the mount contract, what a run hands back — is here as
the reason for it rather than as a second copy of the mechanics, which is how the two came to
disagree about a timeout once.

### Shape

- Standard library only. `go.mod` has no `require` block, so `go install` fetches one module and
  nothing else, and there is no dependency that can go unmaintained under a tool people install to
  run an agent with their own credentials. Where the standard library genuinely has no answer, box
  writes the few lines itself and says why in `docs/` — see [terminals.md](terminals.md).
- Installation is one `go install` line. That is what decides how updating works: a binary cannot
  compare its own bytes with a published copy, so the check compares releases instead. See
  [updates.md](updates.md).
- One installed copy serves every project, so everything resolves relative to the current working
  directory and nothing relative to the binary's own location.
- Everything that touches the outside world — processes, the terminal, the network, the clock —
...
- `config` takes the same flags as `run` and resolves the same settings, so it answers "what would
  run do" and validates the project's files without creating anything. `gen` and `mount-prompt`
  work on those files instead of reading settings, so they reject every flag.
...
- `kit` must name a directory when it names anything on disk, since `sbx` reads a non-directory as
  a zip artifact and fails with a message about zip files. A `kit` that is not on disk is a
...
- `~/.box/deps` — `$XDG_DATA_HOME/box/deps` when that is set — holds what this machine cannot
  supply, such as a Linux toolchain on macOS, since the sandbox runs Linux whatever the host runs
  and a build that exists only to be mounted belongs to the machine, not to any project. It sits
  outside every repository deliberately: `sbx create --clone` wipes the in-container workspace
  before cloning into it, and a mount nested inside the workspace turns that wipe into a deadlock,
  EROFS on every file with a mountpoint that cannot be removed. The agent answering `mount-prompt`
  creates it; box never downloads into it. A dependency reaches the sandbox by being mounted, not
  by sitting in the project, because the container gets a clone and a clone holds no ignored files.
  The host repository is another matter: sbx mounts it read-only at `/run/sandbox/source`, ignored
  files included, so a project directory is no place for a secret even when git never sees it.
...
- `mount-prompt` prints to stdout, and the user hands the prompt to an interactive agent session
  ...
  never to point a mount inside the project. It asks about every declared mount without a path,
  ...
  work; it says so on stderr, so stdout stays nothing but a prompt to hand over.

### Secrets

- The OAuth token path comes from `CLAUDE_OAUTH_TOKEN_FILE` and from nowhere else, so a shared
  project config can never point at someone else's credentials. `BOX_SECRETS_FILE` names the values
  behind `secret_hosts` for the same reason: the config says which variable may reach which host,
  which is the same everywhere, and the file holding the values is this machine's alone.
...
- The sandbox scope reaches `sbx secret` as `--sandbox`, never as a positional argument. sbx 0.38
  deprecates the positional form, and dropping a secret throws its output away, so the day sbx
  removes it a token would outlive its sandbox without a word, scoped to a name box hands out again.
- Store the secret *before* `sbx create`. `sbx` injects the placeholder env var into the sandbox at
  creation time, so a secret stored afterwards leaves `CLAUDE_CODE_OAUTH_TOKEN` unset and the agent
  starts logged out.

### The prompt

- The base prompt lives in `internal/config/prompts.go` and holds only what is true of every
  sandbox. Anything about one project belongs in that project's `prompt_file`, and anything about
  one machine or one host operating system belongs nowhere: the sandbox is Linux, but where a host
  directory is mounted is not something the prompt can assert.
- It tells the agent it is unattended while `box run` opens an *interactive* session, and both are
  deliberate. The session is interactive so the user can watch what the agent does and step in; the
  prompt tells it to assume and keep going because the user may well walk away, and an agent
  waiting on a question they never see wastes the whole run.

### What comes back

- Never remove a sandbox that holds uncommitted work. Its refs are left as they are too, since the
  run is not over: the work is fetched again once the sandbox is clean.
- A `refs/sandboxes/*` ref is where the fetch lands, not where work is left. A ref holding commits
  becomes a branch named by `claude -p`, and is dropped once that branch exists; a ref holding
  nothing new is dropped without one, since there is nothing to name.
- Naming a branch is a courtesy, so every way it can fail — no `claude` on `PATH`, a non-zero exit,
  ten seconds of silence, an empty answer, a name git refuses, a ref git cannot read — keeps the
  ref instead. The work is already fetched, and a kept ref is what the user had before.
...
- A failed `sbx create` is reported, not raised. `sbx` has already said why, there is no sandbox to
  clean up or keep, and the stored secret is dropped again, so a failure leaves nothing behind.
...
- The `sbx` on `PATH` must be 0.38.0 or newer, read from `sbx version` before a command creates or
  ...
  break a working machine over a change in how `sbx` spells its release. The daemon check comes
  after it, since an old sbx need not print the diagnose JSON that check reads.
...
- `box gen` writes a starter kit at `.box/kit/spec.yaml` and points `kit` at it, since a
  hand-written network policy is the biggest step in setting box up and "the agent's own API calls
  and nothing else" is where most projects start. It says in the file that it is a starting point,
  and holds only the few fields box is sure of, because the schema is sbx's and can drift.
- `box gen` never changes a value that is already there, so re-running it cannot lose a config or
  ...
  appends the mounts and repos files to `.gitignore` in every project, group or not.
- It writes a single project's settings and never the keys only a group needs, so a config it
  ...
  takes no flags and a script still gets today's defaults without answering anything. An input that
  ends without an answer writes nothing at all, since guessing at that point is how a folder ends
  up set up as the wrong thing.
...
- A member is declared by a name, its branch and its `git_origin`, and the gitignored
  ...
  supplied. A name with no path, an empty one or one nobody declared is an error, since a member
